...
if __name__=='__main__':
    ...
    agent = Agent(input_dims=nodes*2, env=env, lam=lam,
            lamE=lamE, rng=rng, n_actions=nodes**2,
    ...
    for g in tqdm(range(n_games)):
        ...
        for s in tqdm(range(n_steps)):
            ...
            if not evaluate and s%50 == 49:
                agent.learn()
            observation = observation_
            if idx<20:
                continue
            avg_score = np.mean(History[-1,idx-20:idx])
            if idx == 1000 and not evaluate:
                agent.save_models()
    ...
    if not evaluate:
        ...
        print("Mean E",np.mean(Mean[nodes:2*nodes],axis=1))
        # SHARED
